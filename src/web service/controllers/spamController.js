...
exports.getSpamMails = (req, res) => {
  ...
  try {
      ...
      return res.status(200).json(spamModel.getAllSpamByUser(username));
    } catch (e) {
      // error in the server
      return res
        .status(500)
...
exports.getSpamMailById = (req, res) => {
  const userId = req.userId;
  const { id } = req.params;
...
// Delete a spam mail 
exports.deleteSpamMailById = (req, res) => {
  const userId = req.userId;
  const { id } = req.params;
  ...
  const username = user.username;
  const success = spamModel.finalDeleteSpamById(id, username);
...
exports.restoreSpamMail = (req, res) => {
  const userId = req.userId;
  const { id } = req.params;
  ...
  // const copiedMail = mailModel.createCopyOfMail(mail, username);
    switch (mail.source) {
      case "inbox":
        copiedMail = mailModel.createCopyOfMail(mail);
        destination = "inbox";
        break;
      case "drafts":
        copiedMail = draftModel.createCopyOfDraft(mail);
        destination = "drafts";
        break;
      case "trash":
        copiedMail = trashModel.createCopyOfTrash(mail);
        destination = "trash";
        break;
      default:
        copiedMail = mailModel.createCopyOfMail(mail);
  ...
  const success = spamModel.deleteSpamById(id, username);
  if (!success) {
...
exports.addToSpamFromInbox = (req, res) => {
  const userId = req.userId;
  const { id } = req.params;
  ...
  spamModel.addSpamMailAndBlacklistLinks(mail, "inbox");
...
exports.addToSpamFromDraft = (req, res) => {
  const userId = req.userId;
  const { id } = req.params;
  ...
  try{
    // Add the mail to the spam list
  spamModel.addSpamMailAndBlacklistLinks(mail, "drafts");
  return res.status(201).json({ message: "Mail marked as spam", mail: mail });
  }
    catch (error) {
        return res.status(500).json({ error: "Failed to add mail to spam", detail: error.message || String(error) });
...
// Add a mail from inbox to spam
exports.addToSpamFromTrash = (req, res) => {
  const userId = req.userId;
  const { id } = req.params;
  ...
  try{
    const result = trashModel.deleteTrashById(id, username);
    if (!result) {
  ...
  try{
    // Add the mail to the spam list
  spamModel.addSpamMailAndBlacklistLinks(mail, "trash");
  return res.status(201).json({ message: "Mail marked as spam", mail: mail });
  }
    catch (error) {
        return res.status(500).json({ error: "Failed to add mail to spam", detail: error.message || String(error) });
...
exports.readSpam = (req, res) => {
  ...
  // Check if the mail exists
  const spamExists = spamModel.getSpamById(id, username);
  if (!spamExists) {
    return res.status(404).json({ error: "Spam not found for this user" });
  }

  if (!spamModel.markReadSpam(spamExists)) {
...
//if (!userId) {
  ...
  if (!spamModel.markUnreadSpam(spamExists)) {
...
exports.updateLabelsInSpam = (req, res) => {
  const { id } = req.params;
  ...
  const updated = spamModel.editLabelsInSpam(
    id,
    username,
    uniqueLabels
  );
...
exports.unstarredSpam = (req, res) => {
  ...
  if (!spamModel.markUnstarredSpam(spamExists)) {
...
exports.starredSpam = (req, res) => {
  ...
  if (!spamModel.markStarredSpam(spamExists)) {
